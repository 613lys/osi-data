...
- EntityType relationships use globally unique OSI relationship `name` values in `<ACTION>_<EntityConceptName>` form, such as `REFERENCES_Counterparty`, `DEPENDS_ON_CollateralAsset`, `DERIVES_FROM_PositionData`, or `VALUES_CollateralAsset`; the generated business graph edge id is exactly that relationship name, while the UI edge type and canvas label are only the controlled action prefix.
- Controlled EntityType relationship actions are only: `CREATES`, `REFERENCES`, `DEPENDS_ON`, `DERIVES_FROM`, `AGGREGATES`, `RECONCILES_WITH`, `SETTLES`, `VALUES`, `PART_OF`, `CHILD_OF`, `RELATED_TO`.
