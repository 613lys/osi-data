...
- Report Data Logic may reference metric-related expressions when explaining how existing dataset fields satisfy calculated requirements. When a requirement calculation output is mapped to this metric through an EntityType value relationship, the UI graph can show `Report Data Logic -> Metric` and `Report Data Logic field -> Metric` `IMPLEMENTS_METRIC` edges.
